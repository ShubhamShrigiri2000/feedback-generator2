...
export default function ManageSkills() {
  ...
  useEffect(() => { loadSkillsByDepartment(data.department); /* eslint-disable-line */ }, []);
  useEffect(() => { if (data.department) loadSkillsByDepartment(data.department); }, [data.department]);
  useEffect(() => { if (!successMessage) return; const t = setTimeout(() => setSuccessMessage(""), 3000); return () => clearTimeout(t); }, [successMessage]);

  const loadSkillsByDepartment = async (department) => {
    try {
      setIsLoading(true); setError(null); setValidationErrors([]);
      const res = await skillsService.getSkillsByDepartment(department);
  ...
  const handleSave = async () => {
    try {
      ...
      if (data.id) { await skillsService.updateSkills(data.id, data); setSuccessMessage("✓ Skills updated successfully!"); showSuccess("Skills updated"); }
      else { const created = await skillsService.createSkills(data); setData(created); setSuccessMessage("✓ Skills created successfully!"); showSuccess("Skills created"); }
